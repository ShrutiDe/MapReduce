...
public class ComputeEngineSample {
  ...
  private static String SAMPLE_INSTANCE_NAME;
  ...
  public static void createInstance(String SAMPLE_INSTANCE_NAME, String script) {
    try {
      httpTransport = GoogleNetHttpTransport.newTrustedTransport();

      String jsonPath =String.format("gs://shruti-desai/shruti-desai-1dc41788adaf.json"+script, PROJECT_ID);
  ...
  public static String getIP(String SAMPLE_INSTANCE_NAME) throws IOException {
	    System.out.println("================== Listing Compute Engine Instances ==================");
	    Compute.Instances.List instances = compute.instances().list(PROJECT_ID, ZONE_NAME);
	    InstanceList list = instances.execute();
	  String ip = null;
	      for (Instance instance : list.getItems()) {
	        ...
	        System.out.println(instance.toPrettyString());
	        if (instance.getName().equals(SAMPLE_INSTANCE_NAME)) {
	        	ip=instance.getNetworkInterfaces().get(0).getAccessConfigs().get(3).getNatIP();
	        }
	      }
  ...
  public static Operation startInstance(Compute compute, String instanceName) throws IOException {
    ...
    item.setValue(String.format("gs://shruti-desai/"+script, PROJECT_ID));
  ...
  static void deleteInstances(String name) throws Exception {
	  Compute.Instances.List instances = compute.instances().list(PROJECT_ID, ZONE_NAME);
	    InstanceList list = instances.execute();
	      for (Instance instance : list.getItems()) {
	        ...
	        if (instance.getName().contains(name)) {
	        	Compute.Instances.Delete delete =
	        	        compute.instances().delete(PROJECT_ID, ZONE_NAME, instance.getName());
	        	     delete.execute();
	        }
	      }
  }
